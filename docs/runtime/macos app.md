...
The native desktop surface from [[pibarm runtime PRD]] F3, decision D9 in [[pibarm runtime design]]. "Native" is the requirement, so this is a SwiftUI app speaking the [[runtime core and protocol|protocol]] directly — not a webview shell. The cross-platform alternative was weighed and recorded at D9; Windows/Linux take the other road ([[windows and linux]]).
...
- Connects to `pibarmd` over the unix socket when local (spawning/adopting the launchd agent if not running — the app can own host lifecycle on this platform), WebSocket for remote hosts. Multiple host profiles supported.
...
- **Agent grid**: the [[sessions and multiplexing]] renderer done natively — `NSSplitView`-style resizable grid, ≤3 visible agents per policy, overflow to a separate window after the confirm flow, focus cycling on shortcuts mirroring `/matrix-attach`. Any pane pops out to its own window.
...
- Protocol client as a Swift package (`PibarmKit`): JSON-RPC over `Network.framework`/`URLSessionWebSocketTask`, codable event types generated from the protocol schema so Swift and TS cannot drift silently (schema-first, single source in the host repo).
